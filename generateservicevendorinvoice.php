...
if (isset($_POST['conform'])){
	...
	$pdffilename = trim($companyname.'-'.date("d-M-y").'-'.$ftotamount.'.pdf');
	  ...
	  $sqlinv = "INSERT INTO   ws_goodservice_vendorinvoice_master (`company_id`, `companyname`, `invoicenumber`, `invoicecreateddate`, `invoiceduedate`,  `invoiceamount`, `invoiceoutstanding`, `invoicesubtotal`, `invoicefromdate`, `invoicetodate`, `paidamount`, `pdffilename`,invoicebilleddesc,invoicedisputeemail) 
	...
	for($k=0;$k<count($prefixData);$k++){
		...
		if(  $quantity >0){
			$sqlchd = "INSERT INTO   ws_goodservice_vendorinvoice_child (invmasterid,customerName,company_id,`account_id`, `prefix`,  `Description`, `price_per_1_min`,  `quantity`,  `BilledDuration_min`, `Charged_Amount`,fromDate,toDate) 
