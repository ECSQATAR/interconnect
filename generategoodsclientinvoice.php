...
$sqlp = "SELECT * FROM serviceprefixmaster";
...
$cdate = date("Y-m-d");
$sqlcurntInvcount = "select count(id) as cnt From ws_goodservice_clientinvoice_master WHERE company_id=$company_id AND date(invoicecreateddate) = '$cdate' ";
...
$companyname = $rownewcompany->nameofcompany;
 ...
 $Condition='';
 $sql = "SELECT * from  ws_goodservice_clientinvoice_child WHERE invmasterid = $lastInserId  ";
...
?> 
	...
	<td><input type="text" name="quantity[]"  value=""  id="qty_<?php echo $k; ?>"  />  </td>
	<td>
		<input type="text" name="price_per_1_min[]"   value=""   id="price_<?php echo $k;?>"    />
	</td>
...
if (isset($_POST['conform'])){
	...
	$pdffilename = trim($companyname.'-'.date("d-M-y").'-'.$ftotamount.'.pdf');
	  ...
	  $sqlinv = "INSERT INTO   ws_goodservice_clientinvoice_master (`company_id`, `companyname`, `invoicenumber`, `invoicecreateddate`, `invoiceduedate`,  `invoiceamount`, `invoiceoutstanding`, `invoicesubtotal`, `invoicefromdate`, `invoicetodate`, `paidamount`, `pdffilename`,invoicebilleddesc,invoicedisputeemail) 
	...
	for($k=0;$k<count($prefixData);$k++){
		...
		if(  $quantity >0){
			$sqlchd = "INSERT INTO  ws_goodservice_clientinvoice_child (invmasterid,customerName,company_id,`account_id`, `prefix`,  `Description`, `price_per_1_min`,  `quantity`,  `BilledDuration_min`, `Charged_Amount`,fromDate,toDate) 
...
sleep(5);
header('Location: wsgoods_clientinvoices_list.php');
exit(0); 
}
?>
